...
export function startSurvey() {
    if (state.nodes.length === 0) {
        alert('Please create at least one question before running the survey!');
        return;
    }
    ...
    if (validQuestions.length === 0) {
        alert('Please enter content for at least one question!');
        return;
    }
...
export function showSurveyQuestion(nodeId) {
        ...
        if (isEndNode) {
            ...
            surveyBody.innerHTML = `
                ...
                <div class="mb-8 text-center p-10">
                    <div class="text-xl ${infoStyles.text} ${infoStyles.bg} p-5 rounded-lg border-2 ${infoStyles.border} mb-8 whitespace-pre-wrap">${formatTextWithLineBreaks(node.question || 'Notification has no content')}</div>
                    <div class="mt-8">
    ...
    if (validAnswers.length === 0) {
        if (isEndNode) {
            ...
            surveyBody.innerHTML = `
                ${headerHTML}
                <div class="mb-8">
                    <div class="text-2xl font-semibold text-teal-900 mb-6 leading-snug whitespace-pre-wrap">${formatTextWithLineBreaks(node.question || 'Question has no content')}</div>
                    <div class="mt-8">
                        <button class="bg-teal-700 hover:bg-teal-800 border-2 border-teal-700 hover:border-teal-800 text-white w-full rounded-lg px-5 py-4 text-base cursor-pointer transition-all font-semibold text-base px-8 py-4" onclick="window.showSurveyEndHandler()">
                            ✓ End Survey
                        </button>
    ...
    const pathModifiedWarning = state.isPathModified ? `
        <div class="mb-4 p-3 bg-yellow-50 border-l-4 border-yellow-500 rounded text-sm text-yellow-800">
            <strong>⚠️ Note:</strong> You have changed your selection. A new path will be created from here.
        </div>
    ` : '';
    ...
    if (isMultipleChoice) {
        ...
        surveyBody.innerHTML = `
            ...
            <div class="mb-8">
                <div class="text-2xl font-semibold text-teal-900 mb-6 leading-snug whitespace-pre-wrap">${formatTextWithLineBreaks(node.question || 'Question has no content')}</div>
                <p class="text-sm text-gray-600 mb-4 italic">You can select multiple answers:</p>
                <div class="flex flex-col gap-3 mb-6">
                    ${regularAnswers.map((answer, index) => {
                        ...
                        <label class="flex items-center gap-3 p-4 bg-gray-50 border-2 ${isChecked ? 'border-teal-500 bg-teal-50' : 'border-gray-300'} rounded-lg cursor-pointer hover:bg-gray-100 transition-all">
                            ...
                            ${isChecked ? '<span class="text-xs text-teal-600 font-semibold">(Previously selected)</span>' : ''}
                        </label>
                    `;
                    }).join('')}
                    ${otherAnswer ? `
                    <div class="p-4 bg-gray-50 border-2 border-gray-300 rounded-lg">
                        <label class="text-base text-teal-900 mb-2 block">${otherAnswer.text || 'Other'}:</label>
                        <input type="text" 
                               ...
                               placeholder="${otherAnswer.placeholder || 'other opinion'}"
                ...
                <button class="bg-teal-700 hover:bg-teal-800 text-white w-full rounded-lg px-5 py-4 text-base cursor-pointer transition-all font-semibold ${state.isPathModified ? 'opacity-75' : ''}" 
                        ...
                        ${state.isPathModified ? 'title="You have changed your selection. Please confirm to continue."' : ''}>
                    Continue →
                </button>
            </div>
        `;
            ...
            <div class="mb-8">
                ...
                <div class="flex flex-col gap-3">
                    ${regularAnswers.map((answer, index) => {
                        ...
                        <button class="bg-gray-50 hover:bg-teal-800 hover:border-teal-800 hover:text-white border-2 ${isSelected ? 'border-teal-500 bg-teal-50' : 'border-gray-300'} hover:border-teal-800 rounded-lg px-5 py-4 text-base text-teal-900 hover:text-white cursor-pointer transition-all hover:translate-x-1 text-left font-sans ${!hasLink ? 'opacity-90' : ''}" onclick="window.selectSurveyAnswerHandler('${nodeId}', ${answerIndex})">
                            ...
                            ${isSelected ? '<span class="text-xs text-teal-600 font-semibold ml-2">(Previously selected)</span>' : ''}
                        </button>
                    `;
                    }).join('')}
                    ${otherAnswer ? `
                    <div class="bg-gray-50 border-2 ${restoredIndex === otherAnswerIndex ? 'border-teal-500 bg-teal-50' : 'border-gray-300'} rounded-lg p-4 mb-3">
                        ...
                        ${restoredIndex === otherAnswerIndex ? '<span class="text-xs text-teal-600 font-semibold mt-1 block">(Previously selected)</span>' : ''}
                    </div>
                ...
                ${showNextQuestionHint ? `
                    <div class="mt-4 p-2.5 bg-teal-50 border-l-4 border-teal-600 rounded text-xs text-teal-900">
                        <strong>Note:</strong> Answers with arrows (→) will follow their own links. Other answers will follow the default link.
                    </div>
                ` : ''}
...
export function submitMultipleChoice(nodeId) {
    ...
    if (otherAnswer) {
        ...
        if (otherText) {
            ...
            // Remove old "Other" if exists (without text) and add new one (with text)
...
export function showSurveyEnd() {
    ...
    surveyBody.innerHTML = `
        <div class="text-center p-10">
            <h3 class="text-2xl text-teal-700 mb-4">✓ Survey Completed!</h3>
            <p class="text-base text-gray-500 mb-6">Thank you for participating in the survey.</p>
            <div class="flex gap-4 justify-center mt-6">
                <button class="bg-teal-700 hover:bg-teal-800 text-white px-8 py-3 rounded-md text-base cursor-pointer transition-colors" onclick="window.showReviewAnswersHandler()">📋 Review Answers</button>
                <button class="bg-teal-800 hover:bg-teal-900 text-white px-8 py-3 rounded-md text-base cursor-pointer transition-colors" onclick="window.startSurveyHandler()">Run Again</button>
            </div>
        </div>
    `;
    state.currentSurveyNodeId = null;
        ...
        return `
            <div class="bg-white border-2 border-gray-200 rounded-lg p-5 transition-all hover:border-teal-600 hover:shadow-md">
                <div class="flex items-center gap-2.5 mb-4 pb-2.5 border-b border-gray-200">
                    ...
                    <span class="text-xs text-gray-500 font-semibold uppercase">Question #${nodeIndex}</span>
                </div>
                <div class="text-base font-semibold text-teal-900 mb-3 leading-relaxed whitespace-pre-wrap">${formatTextWithLineBreaks(item.question || 'No content')}</div>
                <div class="text-sm text-teal-800 leading-relaxed p-3 bg-gray-50 rounded-md border-l-4 border-teal-700">
                    <strong class="text-teal-700 mr-2">Selected:</strong> 
    ...
    surveyBody.innerHTML = `
        <div class="p-5">
            <div class="text-center mb-8 pb-5 border-b-2 border-gray-200">
                <h3 class="text-2xl text-teal-900 mb-2.5">📋 Review Answers</h3>
                <p class="text-sm text-gray-500 m-0">Total: ${filteredEntries.length} ${filteredEntries.length === 1 ? 'answer' : 'answers'}</p>
            </div>
            <div class="flex flex-col gap-5 max-h-[calc(90vh-250px)] overflow-y-auto p-2.5">
                ${reviewHTML}
            </div>
            <div class="text-center mt-8">
                <button class="bg-teal-800 hover:bg-teal-900 text-white px-8 py-3 rounded-md text-base cursor-pointer transition-colors" onclick="window.showSurveyEndHandler()">Back</button>
            </div>
        </div>
    `;
}
...
function detectSingleChoiceChange(nodeId, answerIndex, otherText = '') {
    ...
    if (answerIndex >= 0 && node.answers[answerIndex] && node.answers[answerIndex].isOther) {
        ...
        // Extract text after ": " from history (format: "Other: text")
